...
import EllipticCurves.Mathlib.WeierstrassFormalGroup.Filtration
...
* `selmerGroup₂_eq_badPrimes` (over a number field): the local conditions at the good finite
  ...
  `EllipticCurves.Mathlib.WeierstrassFormalGroup.Filtration`.
* `card_range_μ_adicCompletion`, `card_range_μ_completion_isReal`,
  ...
  `(𝒪_v, +)`, proved in `EllipticCurves.Mathlib.WeierstrassFormalGroup.Filtration`).
...
The formal-group input (proved in `EllipticCurves.Mathlib.WeierstrassFormalGroup.Filtration`) is
