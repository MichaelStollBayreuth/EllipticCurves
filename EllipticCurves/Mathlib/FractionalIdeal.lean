...
valuation bookkeeping (`FractionalIdeal.count` and the factorization lemmas in
...
* `FractionalIdeal.exists_pow_eq`: **`n`-th roots** — a fractional ideal all of whose valuations
  ...
  of the Selmer group.
* `FractionalIdeal.prod_count`, `FractionalIdeal.count_injective`: a nonzero fractional ideal is the
  product of `v ^ (count v)` over the primes, and is determined by its valuations.
...
for `n ≠ 0`. In particular the `n`-th root in `FractionalIdeal.exists_pow_eq` is unique. -/
...
/-- The `v`-adic valuation of `x : K` is `exp (- count v (x))`: the `FractionalIdeal.count` of the
...
`FractionalIdeal.valuation_eq_exp_neg_count`. -/
...
/-- The restriction of `toPrincipalIdeal` to `FractionalIdeal.unitsNDivisible`, into the
`n`-divisible subgroup. -/
